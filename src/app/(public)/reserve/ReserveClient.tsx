...
const steps = ['① 날짜 & 인원', '② 예약자 확인', '③ 입금 안내']
    ...
    <>
      ...
      <div className="max-w-[1080px] mx-auto px-12 py-12 grid md:grid-cols-[1fr_360px] gap-10 items-start">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-5">
                    <button
                      ...
                      className="w-9 h-9 border border-[#E5E0D8] flex items-center justify-center text-lg hover:border-[#1C1C1C] transition-colors"
                    >−</button>
          ...
          {step === 2 && (
            <div className="bg-white border border-[#E5E0D8] p-10">
              <div className="text-center mb-8">
                ...
                <p className="text-[14px] text-[#6B6B6B]">예약 번호: <span className="font-mono text-[#1C1C1C]">{reservationId.slice(0, 8).toUpperCase()}</span></p>
              </div>
              <div className="bg-[#FFF8E8] border-l-4 border-[#8B6914] p-6 mb-6">
                <p className="text-[14px] font-medium text-[#8B6914] mb-3">⏱ 입금 기한: 신청 후 2시간 이내</p>
                <p className="text-[13px] text-[#6B6B6B] leading-[1.8]">
                  계좌 정보 및 상세 안내는 <strong>가입하신 이메일</strong>로 발송되었습니다.<br />
                  2시간 이내에 입금하시면 관리자 확인 후 예약이 최종 확정됩니다.
                </p>
              </div>
              <button
                onClick={() => router.push('/mypage')}
                className="w-full bg-[#1C1C1C] text-white py-4 text-[14px] tracking-[0.1em] hover:bg-[#8B6914] transition-colors"
              >
                마이페이지에서 예약 확인하기
              </button>
            </div>
        ...
        <div className="sticky top-6 bg-white border border-[#E5E0D8] p-8">
          ...
          {step === 0 && (
            <button
              onClick={() => nights >= 1 ? handleSubmit() : null}
              disabled={nights < 1 || loading}
